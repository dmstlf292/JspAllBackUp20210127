...
public class PollMgr {//카카오 투표프로그램 처럼....
	...
	//Max num(가장 최신의 투표 설문 리스트가 나타나도록!)
	...
	//Poll Insert (설문과 Item이 동시에 저장이 되게끔...)
	...
	public Vector<String> getItemList(int listNum){//빈즈 안에 벡터!가 있어서 어렵다
		Connection con = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String sql = null;
	...
	//Count Sum (설문 투표수(결과를 보기 위해서)->count)
	public int sumCount (int listNum) {//select - db1
		Connection con = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String sql = null;
	...
	//Poll Update (투표 실행) :매개변수 2개 들어간다. num값과 어떤 아이템, 라디오는 1개만 넘어오지만 체크박스는 아이템 num값 배열로 넘어온다.
	public boolean updatePoll(int listNum, String itemNum[]) {//itemNum이거는 정수로 바꿔줘야한다.
		Connection con = null;
		PreparedStatement pstmt = null;
		String sql = null;
		boolean flag=false;
	...
	//Poll View (결과보기)
	public Vector<PollItemBean> getView(int listNum){//2개의 값이(빈즈에 넣어서 사용할 수 밖에 없음) 동시에 리턴을 하려면 빈즈를 사용할 수 밖에 없다. 1개면 상관없는데...
		Connection con = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String sql = null;
	...
	//Max Item Count : 아이템 중에서 가장 높은 투표수를 가진 값
